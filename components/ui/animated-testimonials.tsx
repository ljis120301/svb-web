...
export const AnimatedTestimonials = ({
  ...
  // Generate deterministic rotations per index to avoid SSR/CSR mismatches
  const rotations = useMemo(() => {
    const count = testimonials.length;
    const values: number[] = [];
    for (let i = 0; i < count; i++) {
      // Simple deterministic pseudo-random based on index
      const seed = (i * 9301 + 49297) % 233280; // LCG-like
      const normalized = seed / 233280; // 0..1
      const angle = Math.floor(normalized * 21) - 10; // -10..10
      values.push(angle);
    }
    return values;
  }, [testimonials]);
